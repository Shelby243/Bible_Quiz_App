...
const QuizScreen = () => {
  ...
  // State to store the shuffled data
  const [shuffledData, setShuffledData] = useState([]);

  useEffect(() => {
    // Shuffle the data when the component first mounts
  ...
  // State to store the current random index
  const [randomIndex, setRandomIndex] = useState(generateRandomIndex());
  // Function to handle the "Next question" button press
  const handleNextQuestion = () => {
    setIndex(index + 1);
    setCounter(15);

    // Generate a new random index when pressing "Next question"
